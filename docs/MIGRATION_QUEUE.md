...
`schema_migration_queue.sha256` has been written since the first version of this
queue, and its own comment says it exists *"so what RAN can be compared to the
repo"*. For as long as it existed, nothing compared it. Measured 12 Sep 2026:
**2 of 55** settled rows already differed from their repository file, and no
surface anywhere said so.

Editing a migration that has already applied is the mistake the pipeline refuses
everywhere else — `enqueue` will not overwrite the SQL of a version it holds,
and the apply workflow prints `::warning title=Modified migration` for a changed
file. But that warning fires only for the push that does it, and only if the
push went through that workflow. Nothing checked afterwards, and nothing checked
a file edited by any other route.

### Two halves, split by one credential

The digests live in Mission Control's database, which this repository cannot
reach. Reading them needs `CRON_SECRET` — the credential that authenticates 32
scheduled workers **and** the endpoint that executes SQL as `postgres`. Putting
that into `ci.yml`, which runs on every pull request, to power a read-only
comparison is not a trade worth making. So:

| | where | what it judges |
| --- | --- | --- |
| `npm run check:applied-digests` | `ci.yml`, every PR and push | the repository against a **committed manifest** — offline, no credential |
| `npm run migrations:digests:check` | `apply-migrations.yml`, which already holds the credential | the **manifest** against the live queue |

`scripts/applied-migration-digests.txt` is the manifest, generated by
`npm run migrations:digests`. The second half is what stops it drifting from the
live table unnoticed — a hand-edited or fabricated entry would otherwise make
the offline check pass against a digest nothing ever ran.

### Four rules

**It judges only what the manifest NAMES.** A migration the queue has not
settled — including one added in the very pull request being checked — has no
digest to be judged against, and judging it would be judging a file against a
row that does not exist.

**It compares BYTES, and a person decides what is excusable.** The two entries
in `applied-digest-baseline.txt` were checked by hand: comments stripped from
both copies, remaining statements byte-identical. That verdict is *recorded*
there rather than *computed* by the checker, because `stripSqlComments` is
conservative and not dollar-quote aware and its failure mode is OVER-stripping —
a "only comments changed" verdict computed that way is the confident-wrong
answer this programme keeps finding.

**An empty digest list is refused, never written.** Zero settled migrations is
not a state this queue can be in, so it is a failed read. Writing it would erase
the manifest and turn the offline check into a no-op that reports success.

**A manifest that is merely BEHIND is a notice, not a failure** — and has to be.
