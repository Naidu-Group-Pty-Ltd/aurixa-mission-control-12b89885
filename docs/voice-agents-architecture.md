...
`raise_support_ticket`) plus the two VAPI-native tools that need no handler,
`end_call_tool` and `transfer_to_human_mc`.

Read [`voice-fleet-capabilities.md`](./voice-fleet-capabilities.md) before
touching `scripts/voice/`. Four reported defects turned out to be three
capabilities that had never been wired and one corpus that was narrow rather
than repetitive — and the deploy script **deleted the end-call tool on every
run while asserting the deletion as its success condition**. Two traps it
records keep biting: `fleet-prompts/*.md` and `manifest.json` are GENERATED by
`build-fleet-prompts.py` (a hand edit is discarded by the next run, and the
manifest is what binds tools), and the knowledge base's prices were typed in
and compared with nothing, so every tier was quoting several hundred dollars
under the list price. Both are now byte-compared by a `--check` mode, and every
price is generated from `src/lib/pricing/aurixa-catalog.ts`.
