...
export type MergeDrainReport = {
   ...
   * pull request had closed — by the per-proposal handling, which clears the
   * notice of any proposal it reads closed, or by
   * `clearNoticesForClosedProposals`, which reads the ones that handling did
   * not. Counted per pull request, not per notice, and only where a notice
   * was actually taken down: a clear that matched nothing is not an event.
   */
...
export async function drainCascadeMerges(
  ...
  */
  ...
  const perClone = async (raw: (typeof eligible)[number]) => {
    ...
    /**
     * The pull requests this pass has READ, which is what the notice sweep
     * below may skip — never `all`. A proposal the cap left out is on the
     * work list and unread, and skipping it too let an alarm about a closed
     * pull request stand for as long as twenty-five newer proposals stayed
     * open ahead of it. Added only once `handleOne` returns: it reads the
     * pull request first, and one that threw may not have got that far.
     */
    ...
    /*
      THE ALARMS OF PROPOSALS THIS PASS DID NOT READ.
      ...
      reporting `ci_red` over them on all three clones for up to six days. The
      same is true of a proposal the per-run cap left out, which is on the
      work list and still unread. Those are looked up here, once each, and
      cleared if the pull request has closed. Only while the budget allows,
      and never a reason to fail the run — a notice left standing is an alarm
...
async function handleOne(args: {
  ...
  cloneId: string;
  /**
   * Told when this pull request's standing blocked notice was taken down, so
   * the run's report counts the clears this path makes as well as the
   * sweep's. A clear is also the only audit trail an alarm leaves when it
   * goes, and a proposal that closed between the list and this read makes no
   * other change the route would file a row for.
   */
  noticeCleared: () => void;
}): Promise<MergeDrainOutcome> {
  const { supabase, octokit, owner, repo, number, rows, label, cloneId, noticeCleared } = args;
  ...
  if (facts.state === "closed") {
    if (await clearBlockedNotifications(supabase, cloneId, number)) noticeCleared();
  }
  if (mergedNow) {
    return {
      clone: label,
 ...
 *
 * Returns whether a notice was taken down — rows came back from the update —
 * so a caller counting what it cleared counts what it did rather than what it
 * tried. Most closed proposals never raised a notice at all, and an update
 * that matched nothing reporting `true` would file an audit row for every one
 * of them.
 */
 ...
 * closed, for the pull requests this pass has not already read.
 *
...
async function clearNoticesForClosedProposals(args: {
  ...
  repo: string;
  cloneId: string;
  /** Pull requests the per-proposal handling read this pass, and so cleared itself. */
  alreadyRead: ReadonlySet<number>;
  isPastDeadline: () => boolean;
}): Promise<number> {
  const { supabase, octokit, owner, repo, cloneId, alreadyRead, isPastDeadline } = args;
  let cleared = 0;
  try {
    const { data, error } = await supabase
      .from("notifications")
    ...
    for (const number of blockedNoticesToRecheck(notices, { owner, repo, alreadyRead })) {
      if (isPastDeadline()) break;
      try {
        const { data: pr } = await octokit.pulls.get({ owner, repo, pull_number: number });
