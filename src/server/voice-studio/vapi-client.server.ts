...
export function vapiClient(apiKey: string): VapiApi {
  const call = async (method: string, path: string, body?: BodyInit, json = true): Promise<Record<string, any> | null> => {
    for (let attempt = 1; ; attempt++) {
      const res = await fetch(`${VAPI_BASE}${path}`, {
        method,
        headers: { Authorization: `Bearer ${apiKey}`, ...(json && body ? { "Content-Type": "application/json" } : {}) },
        body,
      });
      ...
      if (retryable && attempt < ATTEMPTS) {
        ...
        await pause(Number.isFinite(after) && after > 0 ? Math.min(after * 1000, 20_000) : 500 * 2 ** attempt);
        continue;
      }
      const detail = (await res.text()).slice(0, 400);
      throw new VapiHttpError(res.status, `VAPI ${method} ${path.split("?")[0]} answered ${res.status}: ${detail}`);
    }
  };
...
export async function probeVapiKey(apiKey: string): Promise<{ ok: true } | { ok: false; reason: string }> {
  try {
    await vapiClient(apiKey).get("/assistant?limit=1");
    return { ok: true };
  } catch (err) {
    if (err instanceof VapiHttpError && (err.status === 401 || err.status === 403)) {
      return { ok: false, reason: "VAPI refused this key - check it is the org's PRIVATE key, not the public one" };
    }
