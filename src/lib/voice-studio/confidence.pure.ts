...
export function computePlanConfidence(input: {
  ...
  if (checkable > 0) {
    ...
    reasons.push(
      `${input.citations.verified} of ${checkable} checkable citations found verbatim in their source`,
    );
  } else {
    score -= 15;
    reasons.push("No citation could be checked against extracted text");
  }
  if (input.citations.asserted)
    reasons.push(`${input.citations.asserted} citations point at PDFs and are model-asserted`);
  ...
  if (input.gapCount)
    reasons.push(
      `${input.gapCount} question${input.gapCount === 1 ? "" : "s"} the documents did not answer`,
    );
  if (input.openItemCount)
    reasons.push(`${input.openItemCount} open item${input.openItemCount === 1 ? "" : "s"}`);
